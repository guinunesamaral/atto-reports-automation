...
class RelatorioHistoricoCallback(Relatorio, CampanhaAtiva):
    ...
    def execute(self):
        ...
        try:
            logging.info(
                f"{datetime.now()}, Started RelatorioHistoricoCallback")
            self.select_relatorio("HistoricoCallback")
            self.fill_report_form()
            self.save_report()
            self.rename_report()
            logging.info(
                f"{datetime.now()}, Finished RelatorioHistoricoCallback")
    ...
    def rename_report(self):
        file = self.file_manager.latest_downloaded_file(2)
        ...
        new_file_path = self.file_manager.join_paths(
            downloads_path, "HistoricoCallback.CSV")
